...
function maybe (fn) {
  return new Promise(function (resolve, reject) {
    var promise = fn(function (err, res) {
      if (err && reject) {
        reject(err)
      } else if (resolve) {
        resolve(res)
...
module.exports = function (options) {
  ...
  function checkAppSecret (token) {
    if (token.appId && token.appSecret && options.checkAppSecret) {
      return maybe(options.checkAppSecret.bind(null, token.appId, token.appSecret))
  ...
  var checkUserSecret = function (token) {
    if (token.userId && token.userSecret && options.checkUserSecret) {
      return maybe(options.checkUserSecret.bind(null, token.userId, token.userSecret))
        .then(function (isValid) {
          if (isValid) {
            return token
          } else {
  ...
  var checkSession = function (token) {
    if (token.session && options.checkSession) {
      return maybe(options.checkSession.bind(null, token.session))
        .then(function (isValid) {
          if (isValid) {
            return token
          } else {
